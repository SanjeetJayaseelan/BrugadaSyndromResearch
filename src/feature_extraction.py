...
methodology described in the accompanying paper (Section 3.2):
  ...
  1. For each subject and lead, align every detected beat on its R-peak.
  2. Take the per-sample median across beats -> one noise-robust "median beat" per lead.
  3. From each median beat, extract 9 features:
       J_amp, ST40, ST80, ST_slope, R_amp, S_amp, QRS_dur, T_amp, J_to_R_ratio
  4. Across 12 leads -> 108 features, plus 3 global rhythm features
     (HR, RR_std, RR_median) -> 111 features total.

This is an independent re-implementation written for reproducibility. It is not
guaranteed to reproduce the original features.csv bit-for-bit (the exact QRS/J-point
detection heuristics used in the original run were not preserved as code), but it
follows the same clinically-grounded definitions and is validated at the bottom of
this file by rank-correlation against the provided features.csv for the handful of
features discussed in the paper (V2_ST40, V1_ST40, V2_J_amp).
...
if __name__ == "__main__":
    ...
    if args.validate_against:
        ...
        for col in ["V2_ST40", "V1_ST40", "V2_J_amp"]:
            a, b = f"{col}_repro" if f"{col}_repro" in merged else col, f"{col}_orig"
            try:
                repro_col = col.replace("V2_ST40", "V2_ST40").replace("V1_ST40", "V1_ST40")
                c = np.corrcoef(merged[f"{col.split('_')[0]}_ST40_repro"] if "ST40" in col else merged[col],
                                 merged[col + "_orig"] if col + "_orig" in merged else merged[col])[0, 1]
            except Exception:
                c = float("nan")
        print("Validation correlations printed above only if column names align; "
              "see README for how the two pipelines' feature names map to each other.")
